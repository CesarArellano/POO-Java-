import java.awt.Font; 
import java.awt.Color;
import java.awt.GridLayout;
import javax.swing.JLabel;
import javax.swing.JPanel;
import javax.swing.JCheckBox;
import javax.swing.JButton;
import javax.swing.JTextField;
import javax.swing.ButtonGroup;

public class PanelForm extends JPanel
{
	static JLabel EncabezadoA,EncabezadoB,Diagonal,NomMedico,FechaEvaluacion,Sexo,NomPaciente,FechaNac,Edad,Anios,Direccion,TF,TM,Pregunta1,Anios2,Meses,Pregunta2,EstadoCivil,Pregunta3,Nota,NoEstudio,Primaria,Secundaria,Preparatoria,Licenciatura,Postgrado,Pregunta4,Pregunta5,Pregunta6,Pregunta7;
	static JTextField NombreMedico,ApellidoPatMedico,ApellidoMatMedico,DiaEvaluacion,MesEvaluacion,AnioEvaluacion,NomPac,ApPatPac,ApMatPac,DiaNac,MesNac,AnioNac,EdadPac,Calle,NumExterior,NumInterior,Zona,Del_Mun,CP,TelefonoF,TelefonoM,AniosVivienda,MesesVivienda,EntidadOrigen;
	static JCheckBox Masculino,Femenino,Casado,UnionL,Soltero,Separado,Divorciado,Viudo,NoSabe,Num,Num1,Num2,Num3,Num4,Num5,Num6,Num7,Num8,Num9,Num10,Num11,Num12,Num13,Num14,Num15,Num16,Num17,Num18,Num19,Num20,Num21,Num22,Num23,Num24,Num25,OpcionS1,OpcionS2,OpcionN1,OpcionN2,Desempleado,JubiladoS,JubiladoC,Profesionista,Jefe,AmaCasa,Ninguna,Catolica,Cristiana,Judio;
	static JButton Guardar;

	PanelForm()
	{
		setLayout(null);
       	setBackground(new Color(41, 41, 41));
       	Font Fuente = new Font("Ubuntu", 3, 16);
       	Font Fuente2 = new Font("Ubuntu", 1, 14);
        
	    EncabezadoA = new JLabel("A. Datos de Identificación del encuestador");
	    EncabezadoA.setForeground(Color.white);
	    EncabezadoA.setFont(Fuente); 
	    EncabezadoA.setBounds(25,25,350,25);

	    NomMedico = new JLabel("Nombre del médico y/o nutriólogo:");
	    NomMedico.setForeground(Color.white);
	    NomMedico.setFont(Fuente2); 
	    NomMedico.setBounds(25,60,300,25);
	    ApellidoPatMedico = new JTextField("Apellido Paterno",15);
	    ApellidoPatMedico.setBounds(280,60,150,25);
	    ApellidoMatMedico = new JTextField("Apellido Materno",15);
	    ApellidoMatMedico.setBounds(450,60,150,25);
	    NombreMedico = new JTextField("Nombre(s)",15);
	    NombreMedico.setBounds(620,60,150,25);
	    FechaEvaluacion = new JLabel("Fecha de Evaluación:");
	    FechaEvaluacion.setForeground(Color.white);
	    FechaEvaluacion.setFont(Fuente2); 
	    FechaEvaluacion.setBounds(790,60,300,25);
	    DiaEvaluacion = new JTextField("dd",2);
	    DiaEvaluacion.setBounds(950,60,50,25);
	    MesEvaluacion = new JTextField("mm",2);
	    MesEvaluacion.setBounds(1020,60,50,25);
	    AnioEvaluacion = new JTextField("aaaa",4);
	    AnioEvaluacion.setBounds(1090,60,50,25);

	    EncabezadoB = new JLabel("B. Cédula de identificación del paciente");
	    EncabezadoB.setForeground(Color.white);
	    EncabezadoB.setFont(Fuente); 
	    EncabezadoB.setBounds(25,100,300,25);

	    NomPaciente = new JLabel("B.1 Nombre:");
	    NomPaciente.setForeground(Color.white);
	    NomPaciente.setFont(Fuente2);
	    NomPaciente.setBounds(25,135,100,25);
	    ApPatPac = new JTextField("Apellido Paterno",15);
	    ApPatPac.setBounds(130,135,150,25);
	    ApMatPac = new JTextField("Apellido Materno",15);
	    ApMatPac.setBounds(300,135,150,25);
	    NomPac = new JTextField("Nombre(s)",15);
	    NomPac.setBounds(470,135,150,25);

	    Sexo = new JLabel("B.2 Sexo:");
	    Sexo.setForeground(Color.white);
	    Sexo.setFont(Fuente2); 
	    Sexo.setBounds(640,135,300,25);
	    Masculino = new JCheckBox("Hombre");
	    Masculino.setForeground(Color.white);
		Masculino.setBackground(new Color(100, 100, 100));
		Masculino.setBounds(720,135,100,25);
		Femenino = new JCheckBox("Mujer");
	    Femenino.setForeground(Color.white);
		Femenino.setBackground(new Color(100, 100, 100));
		Femenino.setBounds(840,135,100,25);

		FechaNac = new JLabel("B.3 Fecha de Nacimiento:");
	    FechaNac.setForeground(Color.white);
	    FechaNac.setFont(Fuente2); 
	    FechaNac.setBounds(25,170,300,25);
	    DiaNac = new JTextField("dd",2);
	    DiaNac.setBounds(215,170,50,25);
	    MesNac = new JTextField("mm",2);
	    MesNac.setBounds(285,170,50,25);
	    AnioNac = new JTextField("aaaa",4);
	    AnioNac.setBounds(355,170,50,25);

	    Edad = new JLabel("B.4 Edad:                          Años");
	    Edad.setForeground(Color.white);
	    Edad.setFont(Fuente2); 
	    Edad.setBounds(425,170,300,25);
	    EdadPac = new JTextField("",3);
	    EdadPac.setBounds(505,170,50,25);

	    Direccion = new JLabel("B.5 Direccion:");
	    Direccion.setForeground(Color.white);
	    Direccion.setFont(Fuente2); 
	    Direccion.setBounds(25,205,300,25);
	    Calle = new JTextField("Calle",50);
	    Calle.setBounds(140,205,250,25);
	    NumExterior = new JTextField("No. Exterior",4);
	    NumExterior.setBounds(410,205,80,25);
	    NumInterior = new JTextField("No. Interior",4);
	    NumInterior.setBounds(510,205,80,25);
	    Zona = new JTextField("Zona",4);
	    Zona.setBounds(610,205,250,25);
	    Del_Mun = new JTextField("Delegación/Municipio",50);
	    Del_Mun.setBounds(880,205,150,25);
	    CP = new JTextField("Código Postal",50);
	    CP.setBounds(1050,205,90,25);

	    TF = new JLabel("B.6 Teléfono Fijo (casa):");
	    TF.setForeground(Color.white);
	    TF.setFont(Fuente2); 
	    TF.setBounds(25,240,300,25);
	    TelefonoF = new JTextField("",50);
	    TelefonoF.setBounds(200,240,150,25);
	    
	    TM = new JLabel("B.7 Teléfono Móvil:");
	    TM.setForeground(Color.white);
	    TM.setFont(Fuente2); 
	    TM.setBounds(370,240,300,25);
	    TelefonoM = new JTextField("",50);
	    TelefonoM.setBounds(520,240,150,25);
	    
	    Pregunta1 = new JLabel("B.8 ¿Cuánto tiempo lleva viviendo en este domicilio? (indique meses o años)");
	    Pregunta1.setForeground(Color.white);
	    Pregunta1.setFont(Fuente2); 
	    Pregunta1.setBounds(25,275,500,25);
	    AniosVivienda = new JTextField("Años",50);
	    AniosVivienda.setBounds(540,275,50,25);
	    MesesVivienda = new JTextField("Meses",50);
	    MesesVivienda.setBounds(610,275,50,25);

	    Pregunta2 = new JLabel("B.9 Estado/Entidad de Origen");
	    Pregunta2.setForeground(Color.white);
	    Pregunta2.setFont(Fuente2); 
	    Pregunta2.setBounds(680,275,300,25);
	    EntidadOrigen = new JTextField("",50);
	    EntidadOrigen.setBounds(890,275,150,25);

	    EstadoCivil = new JLabel("B.10 Estado Civil:");
	    EstadoCivil.setForeground(Color.white);
	    EstadoCivil.setFont(Fuente2); 
	    EstadoCivil.setBounds(25,310,300,25);
	    Casado = new JCheckBox("Casado");
	    Casado.setForeground(Color.white);
		Casado.setBackground(new Color(100, 100, 100));
		Casado.setBounds(160,310,110,25);
		UnionL = new JCheckBox("Unión Libre");
	    UnionL.setForeground(Color.white);
		UnionL.setBackground(new Color(100, 100, 100));
		UnionL.setBounds(290,310,110,25);
		Soltero = new JCheckBox("Soltero");
	    Soltero.setForeground(Color.white);
		Soltero.setBackground(new Color(100, 100, 100));
		Soltero.setBounds(420,310,110,25);
		Separado = new JCheckBox("Separado");
	    Separado.setForeground(Color.white);
		Separado.setBackground(new Color(100, 100, 100));
		Separado.setBounds(550,310,110,25);
		Divorciado = new JCheckBox("Divorciado");
	    Divorciado.setForeground(Color.white);
		Divorciado.setBackground(new Color(100, 100, 100));
		Divorciado.setBounds(680,310,110,25);
		Viudo = new JCheckBox("Viudo");
	    Viudo.setForeground(Color.white);
		Viudo.setBackground(new Color(100, 100, 100));
		Viudo.setBounds(810,310,110,25);
		NoSabe = new JCheckBox("No Sabe / No Responde");
	    NoSabe.setForeground(Color.white);
		NoSabe.setBackground(new Color(100, 100, 100));
		NoSabe.setBounds(940,310,200,25);
		
		Pregunta3 = new JLabel("B.11 ¿Qué nivel de estudios tiene?");
	    Pregunta3.setForeground(Color.white);
	    Pregunta3.setFont(Fuente2); 
	    Pregunta3.setBounds(25,345,300,25);
	    Nota = new JLabel("(Pase al item B.14)");
	    Nota.setForeground(Color.white);
	    Nota.setFont(Fuente2); 
	    Nota.setBounds(270,345,300,25);
	    NoEstudio = new JLabel("No estudió:");
	    NoEstudio.setForeground(Color.white);
	    NoEstudio.setFont(Fuente2); 
	    NoEstudio.setBounds(420,345,300,25);
	    Num = new JCheckBox("0");
	    Num.setForeground(Color.white);
		Num.setBackground(new Color(100, 100, 100));
		Num.setBounds(570,345,50,25);
		Primaria = new JLabel("Primaria en años:");
	    Primaria.setForeground(Color.white);
	    Primaria.setFont(Fuente2); 
	    Primaria.setBounds(640,345,300,25);
	    Num1 = new JCheckBox("1");
	    Num1.setForeground(Color.white);
		Num1.setBackground(new Color(100, 100, 100));
		Num1.setBounds(780,345,50,25);
		Num2 = new JCheckBox("2");
	    Num2.setForeground(Color.white);
		Num2.setBackground(new Color(100, 100, 100));
		Num2.setBounds(850,345,50,25);
		Num3 = new JCheckBox("3");
	    Num3.setForeground(Color.white);
		Num3.setBackground(new Color(100, 100, 100));
		Num3.setBounds(920,345,50,25);
		Num4 = new JCheckBox("4");
	    Num4.setForeground(Color.white);
		Num4.setBackground(new Color(100, 100, 100));
		Num4.setBounds(990,345,50,25);
		Num5 = new JCheckBox("5");
	    Num5.setForeground(Color.white);
		Num5.setBackground(new Color(100, 100, 100));
		Num5.setBounds(1060,345,50,25);
		Num6 = new JCheckBox("6");
	    Num6.setForeground(Color.white);
		Num6.setBackground(new Color(100, 100, 100));
		Num6.setBounds(1130,345,50,25);

		Secundaria = new JLabel("Secundaria en años:");
	    Secundaria.setForeground(Color.white);
	    Secundaria.setFont(Fuente2); 
	    Secundaria.setBounds(420,380,300,25);
	    Num7 = new JCheckBox("7");
	    Num7.setForeground(Color.white);
		Num7.setBackground(new Color(100, 100, 100));
		Num7.setBounds(570,380,50,25);
		Num8 = new JCheckBox("8");
	    Num8.setForeground(Color.white);
		Num8.setBackground(new Color(100, 100, 100));
		Num8.setBounds(640,380,50,25);
		Num9 = new JCheckBox("9");
	    Num9.setForeground(Color.white);
		Num9.setBackground(new Color(100, 100, 100));
		Num9.setBounds(710,380,50,25);
		Preparatoria = new JLabel("Preparatoria (téc.) en años:");
	    Preparatoria.setForeground(Color.white);
	    Preparatoria.setFont(Fuente2); 
	    Preparatoria.setBounds(780,380,300,25);
	    Num10 = new JCheckBox("10");
	    Num10.setForeground(Color.white);
		Num10.setBackground(new Color(100, 100, 100));
		Num10.setBounds(990,380,50,25);
		Num11 = new JCheckBox("11");
	    Num11.setForeground(Color.white);
		Num11.setBackground(new Color(100, 100, 100));
		Num11.setBounds(1060,380,50,25);
		Num12 = new JCheckBox("12");
	    Num12.setForeground(Color.white);
		Num12.setBackground(new Color(100, 100, 100));
		Num12.setBounds(1130,380,50,25);
		Licenciatura = new JLabel("Licenciatura en años:");
	    Licenciatura.setForeground(Color.white);
	    Licenciatura.setFont(Fuente2); 
	    Licenciatura.setBounds(420,415,300,25);
	    Num13 = new JCheckBox("13");
	    Num13.setForeground(Color.white);
		Num13.setBackground(new Color(100, 100, 100));
		Num13.setBounds(570,415,50,25);
		Num14 = new JCheckBox("14");
	    Num14.setForeground(Color.white);
		Num14.setBackground(new Color(100, 100, 100));
		Num14.setBounds(640,415,50,25);
		Num15 = new JCheckBox("15");
	    Num15.setForeground(Color.white);
		Num15.setBackground(new Color(100, 100, 100));
		Num15.setBounds(710,415,50,25);
		Num16 = new JCheckBox("16");
	    Num16.setForeground(Color.white);
		Num16.setBackground(new Color(100, 100, 100));
		Num16.setBounds(780,415,50,25);
		Num17 = new JCheckBox("17");
	    Num17.setForeground(Color.white);
		Num17.setBackground(new Color(100, 100, 100));
		Num17.setBounds(850,415,50,25);
		Num18 = new JCheckBox("18");
	    Num18.setForeground(Color.white);
		Num18.setBackground(new Color(100, 100, 100));
		Num18.setBounds(920,415,50,25);
		Postgrado = new JLabel("Postgrado en años:");
	    Postgrado.setForeground(Color.white);
	    Postgrado.setFont(Fuente2); 
	    Postgrado.setBounds(420,450,300,25);
	    Num19 = new JCheckBox("19");
	    Num19.setForeground(Color.white);
		Num19.setBackground(new Color(100, 100, 100));
		Num19.setBounds(570,450,50,25);
		Num20 = new JCheckBox("20");
	    Num20.setForeground(Color.white);
		Num20.setBackground(new Color(100, 100, 100));
		Num20.setBounds(640,450,50,25);
		Num21 = new JCheckBox("21");
	    Num21.setForeground(Color.white);
		Num21.setBackground(new Color(100, 100, 100));
		Num21.setBounds(710,450,50,25);
		Num22 = new JCheckBox("22");
	    Num22.setForeground(Color.white);
		Num22.setBackground(new Color(100, 100, 100));
		Num22.setBounds(780,450,50,25);
		Num23 = new JCheckBox("23");
	    Num23.setForeground(Color.white);
		Num23.setBackground(new Color(100, 100, 100));
		Num23.setBounds(850,450,50,25);
		Num24 = new JCheckBox("24");
	    Num24.setForeground(Color.white);
		Num24.setBackground(new Color(100, 100, 100));
		Num24.setBounds(920,450,50,25);
		Num25 = new JCheckBox("25");
	    Num25.setForeground(Color.white);
		Num25.setBackground(new Color(100, 100, 100));
		Num25.setBounds(990,450,50,25);	

		Pregunta4 = new JLabel("B.12 ¿Sabes Leer?");
	    Pregunta4.setForeground(Color.white);
	    Pregunta4.setFont(Fuente2); 
	    Pregunta4.setBounds(25,485,300,25);	
	    OpcionS1 = new JCheckBox("Si");
	    OpcionS1.setForeground(Color.white);
		OpcionS1.setBackground(new Color(100, 100, 100));
		OpcionS1.setBounds(160,485,50,25);
		OpcionN1 = new JCheckBox("No");
	    OpcionN1.setForeground(Color.white);
		OpcionN1.setBackground(new Color(100, 100, 100));
		OpcionN1.setBounds(230,485,50,25);
	    
	    Pregunta5 = new JLabel("B.13 ¿Sabes escribir?");
	    Pregunta5.setForeground(Color.white);
	    Pregunta5.setFont(Fuente2); 
	    Pregunta5.setBounds(300,485,300,25);
	    OpcionS2 = new JCheckBox("Si");
	    OpcionS2.setForeground(Color.white);
		OpcionS2.setBackground(new Color(100, 100, 100));
		OpcionS2.setBounds(455,485,50,25);
		OpcionN2 = new JCheckBox("No");
	    OpcionN2.setForeground(Color.white);
		OpcionN2.setBackground(new Color(100, 100, 100));
		OpcionN2.setBounds(525,485,50,25);
		
		Pregunta6 = new JLabel("B.14 ¿A qué se dedica actualmente?");
	    Pregunta6.setForeground(Color.white);
	    Pregunta6.setFont(Fuente2); 
	    Pregunta6.setBounds(25,520,300,25);
	    Desempleado = new JCheckBox("Desempleado");
	    Desempleado.setForeground(Color.white);
		Desempleado.setBackground(new Color(100, 100, 100));
		Desempleado.setBounds(280,520,200,25);
		JubiladoS = new JCheckBox("Jubilado(a) sin pensión");
	    JubiladoS.setForeground(Color.white);
		JubiladoS.setBackground(new Color(100, 100, 100));
		JubiladoS.setBounds(500,520,200,25);
		JubiladoC = new JCheckBox("Jubilado(a) con pensión");
	    JubiladoC.setForeground(Color.white);
		JubiladoC.setBackground(new Color(100, 100, 100));
		JubiladoC.setBounds(720,520,200,25);
		Profesionista = new JCheckBox("Profesionista independiente");
	    Profesionista.setForeground(Color.white);
		Profesionista.setBackground(new Color(100, 100, 100));
		Profesionista.setBounds(940,520,220,25);
		Jefe = new JCheckBox("Patrón(a) jefe(a) o empresario(a)");
	    Jefe.setForeground(Color.white);
		Jefe.setBackground(new Color(100, 100, 100));
		Jefe.setBounds(280,555,420,25);
		AmaCasa = new JCheckBox("Ama de casa");
	    AmaCasa.setForeground(Color.white);
		AmaCasa.setBackground(new Color(100, 100, 100));
		AmaCasa.setBounds(720,555,200,25);
		
		Pregunta7 = new JLabel("B.15 ¿De qué religión es usted?");
	    Pregunta7.setForeground(Color.white);
	    Pregunta7.setFont(Fuente2); 
	    Pregunta7.setBounds(25,590,300,25);
	    Ninguna = new JCheckBox("Ninguna");
	    Ninguna.setForeground(Color.white);
		Ninguna.setBackground(new Color(100, 100, 100));
		Ninguna.setBounds(280,590,200,25);
		Catolica = new JCheckBox("Católica");
	    Catolica.setForeground(Color.white);
		Catolica.setBackground(new Color(100, 100, 100));
		Catolica.setBounds(500,590,200,25);
		Cristiana = new JCheckBox("Cristiana");
	    Cristiana.setForeground(Color.white);
		Cristiana.setBackground(new Color(100, 100, 100));
		Cristiana.setBounds(720,590,200,25);
		Judio = new JCheckBox("Judío");
	    Judio.setForeground(Color.white);
		Judio.setBackground(new Color(100, 100, 100));
		Judio.setBounds(940,590,220,25);




	    Guardar = new JButton("Guardar Información");
	    Guardar.setBounds(940,625,220,25);

	    ButtonGroup Grupo1 = new ButtonGroup();
	    ButtonGroup Grupo2 = new ButtonGroup();
	    ButtonGroup Grupo3 = new ButtonGroup();
	    ButtonGroup Grupo4 = new ButtonGroup();
	    ButtonGroup Grupo5 = new ButtonGroup();
	    ButtonGroup Grupo6 = new ButtonGroup();
	    ButtonGroup Grupo7 = new ButtonGroup();

	    Grupo1.add(Masculino);
	    Grupo1.add(Femenino);

	    Grupo2.add(Casado);
	    Grupo2.add(UnionL);
	    Grupo2.add(Soltero);
	    Grupo2.add(Separado);
	    Grupo2.add(Divorciado);
	    Grupo2.add(Viudo);
	    Grupo2.add(NoSabe);

	    Grupo3.add(Num);
	    Grupo3.add(Num1);
	    Grupo3.add(Num2);
	    Grupo3.add(Num3);
	    Grupo3.add(Num4);
	    Grupo3.add(Num5);
	    Grupo3.add(Num6);
	    Grupo3.add(Num7);
	    Grupo3.add(Num8);
	    Grupo3.add(Num9);
	    Grupo3.add(Num10);
	    Grupo3.add(Num11);
	    Grupo3.add(Num12);
	    Grupo3.add(Num13);
	    Grupo3.add(Num14);
	    Grupo3.add(Num15);
	    Grupo3.add(Num16);
	    Grupo3.add(Num17);
	    Grupo3.add(Num18);
	    Grupo3.add(Num19);
	    Grupo3.add(Num20);
	    Grupo3.add(Num21);
	    Grupo3.add(Num22);
	    Grupo3.add(Num23);
	    Grupo3.add(Num24);
	    Grupo3.add(Num25);

	    Grupo4.add(OpcionS1);
	    Grupo4.add(OpcionN1);

	    Grupo5.add(OpcionS2);
	    Grupo5.add(OpcionN2);

	    Grupo6.add(Desempleado);
	    Grupo6.add(JubiladoS);
	    Grupo6.add(JubiladoC);
	    Grupo6.add(Profesionista);
	    Grupo6.add(Jefe);
	    Grupo6.add(AmaCasa);

	    Grupo7.add(Ninguna);
	    Grupo7.add(Catolica);
	    Grupo7.add(Cristiana);
	    Grupo7.add(Judio);

	    add(EncabezadoA);
	    add(NomMedico);
	    add(ApellidoPatMedico);
	    add(ApellidoMatMedico);
	    add(NombreMedico);
	    add(FechaEvaluacion);
	    add(DiaEvaluacion);
	    add(MesEvaluacion);
	    add(AnioEvaluacion);
	    add(EncabezadoB);
	    add(NomPaciente);
	    add(ApPatPac);
	    add(ApMatPac);
	    add(NomPac);
	    add(Sexo);
	    add(Masculino);
	    add(Femenino);
	    add(FechaNac);
	    add(DiaNac);
	    add(MesNac);
	    add(AnioNac);
	    add(Edad);
	    add(EdadPac);
	    add(Direccion);
	    add(Calle);
	    add(NumExterior);
	    add(NumInterior);
	    add(Zona);
	    add(Del_Mun);
	    add(CP);
	    add(TF);
	    add(TelefonoF);
	    add(TM);
	    add(TelefonoM);
	    add(Pregunta1);
	    add(AniosVivienda);
	    add(MesesVivienda);
	    add(Pregunta2);
	    add(EntidadOrigen);
	    add(EstadoCivil);
	    add(Casado);
	    add(UnionL);
	    add(Soltero);
	    add(Separado);
	    add(Divorciado);
	    add(Viudo);
	    add(NoSabe);
	    add(Pregunta3);
	    add(Nota);
	    add(NoEstudio);
	    add(Num);
	    add(Primaria);
	    add(Num1);
	    add(Num2);
	    add(Num3);
	    add(Num4);
	    add(Num5);
	    add(Num6);
	    add(Secundaria);
	    add(Num7);
	    add(Num8);
	    add(Num9);
	    add(Preparatoria);
	    add(Num10);
	    add(Num11);
	    add(Num12);
	    add(Licenciatura);
	    add(Num13);
	    add(Num14);
	    add(Num15);
	    add(Num16);
	    add(Num17);
	    add(Num18);
	    add(Postgrado);
	    add(Num19);
	    add(Num20);
	    add(Num21);
	    add(Num22);
	    add(Num23);
	    add(Num24);
	    add(Num25);
	    add(Pregunta4);
	    add(OpcionS1);
	    add(OpcionN1);
	    add(Pregunta5);
	    add(OpcionS2);
	    add(OpcionN2);
	    add(Pregunta6);
	    add(Desempleado);
	    add(JubiladoS);
	    add(JubiladoC);
	    add(Profesionista);
	    add(Jefe);
	    add(AmaCasa);
	    add(Pregunta7);
	    add(Ninguna);
	    add(Catolica);
	    add(Cristiana);
	    add(Judio);
		add(Guardar);
	}
}
